                        ...
                        @hasanyrole(['peraku'])
                            @if($cekPerakuProjek)
                                <button type="button" class="btn btn-xs btn-warning" id="hantarProjek">
                                    <span id="perakuButton"></span> Permohonan Diperakukan <i class="fa fa-envelope"></i>
                                </button>
                            @endif
                            <button type="button" class="btn btn-xs btn-primary" id="perakuProjek">
                                Diperakukan <i class="fa fa-check"></i>
                            </button>
                        @endhasanyrole
                    @endif
                </div>
				...
				<th width="5%" class="text-center">Keutamaan</th>
                        ...
                        @foreach ($projek as $proj)
                            <tr>
                                ...
                                <td class="text-center">
                                    <a href="/permohonan/baru/perakuan/papar/{{ $proj->projek_id }}" class="btn btn-default btn-xs" title="Papar"><i class="fa fa-search text-warning"></i></a>
                                </td>
                            </tr>
                        @endforeach
                        <tbody>
...
<script>
    ...
    $(document).ready(function(){
        ...
        $('#checkAll').change(function() {
            $('.peraku-checkbox').prop('checked', this.checked);
        });

        $(document).on('click', '#perakuProjek', function (e) {
            e.preventDefault();

            let selectedProjek = [];
            $('.peraku-checkbox:checked').each(function() {
            ...
            if (selectedProjek.length > 0) {
                $.ajax({
                    url: '/permohonan/baru/perakuan-pukal',
                    method: 'POST',
                    data: {
                    ...
                    success: function (response) {
                        if (response.status === 200) {
                            window.location.href = '/permohonan/baru/perakuan';
                        }
                    }
                });
            }
       ...
       // HANTAR UNTUK PERAKUAN
        $(document).on('click', '#hantarProjek', function (e) {
            ...
            $.ajax({
                type: "post",
                url: "/permohonan/baru/maklum-perakuan",
                data: null,
                success: function (response) {
