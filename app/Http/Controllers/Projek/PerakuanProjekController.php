...
class PerakuanProjekController extends Controller
{
    ...
    public function index(){
        ...
        $data['jumlah'] = $jumlah;
        $data['projek'] = $projek;
        $data['cekPerakuProjek'] = $this->cekPerakuProjek();
        $data['cekCompleteProjek'] = $this->cekCompleteProjek();

        // dd($data);
        return view('app.projek-baru.perakuan.index', $data);
    }

    public function view(string $id){
        $data['projek'] = ProjekBaru::find($id);
        return view('app.projek-baru.perakuan.papar', $data);
    }

    public function perakuSatu(Request $req){
        // dd($req->all());
        if($req->proj_wf_sah==1){
            return redirect('/permohonan/baru/perakuan')
            ->with([
                'title'=>'Pengesahan',
        ...
        $projek->proj_wf_sah = $req->proj_wf_sah;
        $projek->save();
         ...
         if($projek){
            ...
            ->with([
                'title'=>'Berjaya',
                'msg'=>'Maklumat projek telah diperakukan',
                'type'=>'success'
            ]);
        }
        else{
            ...
            ->with([
                'title'=>'Gagal',
                'msg'=>'Maklumat projek gagal diperakukan',
                'type'=>'error'
            ]);
        }
    }

    public function perakuPukal(Request $req){
        $projekID = $req->projek;
